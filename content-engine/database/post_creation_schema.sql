-- Post Creation System Schema
-- Supports both All-at-Once and By-Parts modes
...
CREATE TABLE IF NOT EXISTS posts (
    ...
    -- All-at-Once mode content (when creation_mode = 'all_at_once')
    full_content TEXT,
    full_visual_url TEXT,
    full_visual_alt_text TEXT,
    
    -- By-Parts mode content (when creation_mode = 'by_parts')
    -- Individual sections are stored in post_sections table
...
CREATE TABLE IF NOT EXISTS post_sections (
    ...
    ai_model VARCHAR(50), -- 'claude', 'gpt-4', 'midjourney', etc.
    ai_prompt TEXT,
    ai_confidence DECIMAL(3,2), -- 0.00 to 1.00
    
    -- Section status
    is_locked BOOLEAN DEFAULT FALSE, -- Prevents AI from overriding
...
CREATE TABLE IF NOT EXISTS post_generation_history (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    post_id UUID NOT NULL REFERENCES posts(id) ON DELETE CASCADE,
    section_id UUID REFERENCES post_sections(id) ON DELETE CASCADE,
...
CREATE TRIGGER update_post_sections_updated_at BEFORE UPDATE ON post_sections
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
